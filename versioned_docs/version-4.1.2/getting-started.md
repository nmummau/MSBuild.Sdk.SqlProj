...
> Note: For PackageReferences this SDK currently assumes that the `.dacpac` file has the same name as the package. If you plan to create a NuGet package out of your project (see [Packaging and Publishing](packaging-and-publishing.md#packaging-support)) then make sure that `<TargetName>` matches the ID of your package.
